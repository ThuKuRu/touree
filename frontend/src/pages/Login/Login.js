  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      username: Yup.string()
        ...
        .min(1, "Must be between 1 to 50 characters")
        .required("Required!!!"),
      password: Yup.string()
        .max(50, "Must be between 6 to 50 characters")
        .min(1, "Must be between 6 to 50 characters")
        .required("Required!!!"),
    }),
    onSubmit: async (values) => {
      try {
        ...
        } // Lấy status từ phản hồi của API
        // Tiếp tục xử lý phản hồi ở đây nếu cần thiết
      } catch (error) {
        console.log(error);
        openNotificationWithIcon(
          "ERROR",
